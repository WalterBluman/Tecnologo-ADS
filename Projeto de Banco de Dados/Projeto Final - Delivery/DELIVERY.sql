...
BEGIN
	IF (NOT EXISTS(SELECT * FROM PEDIDO WHERE ID_PEDIDO=$2)) THEN
	  RAISE 'PEDIDO NAO EXISTE';
	ELSIF ($1 NOT LIKE 'A' AND $1 NOT LIKE 'C' AND $1 NOT LIKE 'F') THEN
	  RAISE 'STATUS DO PEDIDO INVALIDO';
	END IF;
...
BEGIN
  ...
  IF (NOT EXISTS(SELECT * FROM PRATO WHERE nome = $2)) THEN
    RAISE 'PRATO % NAO EXISTE',$2;
  ELSIF (NOT EXISTS(SELECT * FROM RESTAURANTE WHERE nome = $3)) THEN
    RAISE 'RESTAURANTE % NAO EXISTE',$3;
  ELSIF (NOT EXISTS(SELECT * FROM USUARIO WHERE nome = $5)) THEN
    RAISE 'USUARIO % NAO EXISTE',$5;
  ELSIF (NOT EXISTS(SELECT * FROM ENTREGADOR WHERE nome = $6)) THEN
  RAISE 'ENTREGADOR % NAO EXISTE',$6;
  END IF;
...
BEGIN
	IF (NOT EXISTS(SELECT * FROM PEDIDO WHERE ID_PEDIDO=$1)) THEN
	  RAISE 'PEDIDO NAO EXISTE';
	END IF;
...
SELECT INSERIR('RESTAURANTE', '''ITALLY'', ''9999999'', ''999999999'', 1')
SELECT INSERIR('PRATO', '''SUSHI'', ''GOOD''')
SELECT INSERIR('PRATO', '''PIZZA'', ''GOOD''')
SELECT INSERIR('ESTOQUE', '2, 3, 10, 4')
...
SELECT DELETAR('PEDIDO', 1)
DELETE FROM ITEM_PEDIDO WHERE ID_PEDIDO=8; 
SELECT DELETAR('ENTREGADOR', 'Fulano')


SELECT realiza_pedido(9,'SUSHI','AIKO',1,'EVALDO','unknown')
